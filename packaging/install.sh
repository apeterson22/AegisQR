...
Options:
  ...
  --archive-sha256 hex    Expected SHA256 for --archive; required for remote archives unless --skip-checksum is used
...
remote_archive_requires_https() {
  url=$1
  case "$url" in
    https://*) ;;
    http://*)
      printf 'error: remote archives must use https: %s\n' "$url" >&2
      exit 1
      ;;
    *)
      printf 'error: unsupported remote archive URL: %s\n' "$url" >&2
      exit 1
      ;;
  esac
}
...
find_bundle_dir() {
  bundle_dirs=$(find "$1" -mindepth 1 -maxdepth 1 -type d -name 'aegisqr-*' | sort)
  bundle_count=$(printf '%s\n' "$bundle_dirs" | grep -c . || true)
  if [ "$bundle_count" -ne 1 ]; then
    printf 'error: expected exactly one bundle directory named aegisqr-*, found %s\n' "$bundle_count" >&2
    exit 1
  fi
  ...
  if [ -n "$extra_entries" ]; then
    printf 'error: archive contains unexpected top-level entries outside aegisqr-*\n' >&2
    printf '%s\n' "$extra_entries" >&2
    exit 1
  fi
...
if [ -n "$ARCHIVE" ]; then
  ...
  case "$ARCHIVE" in
    http://*|https://*)
      remote_archive_requires_https "$ARCHIVE"
      if [ "$SKIP_CHECKSUM" -eq 0 ] && [ -z "$ARCHIVE_SHA256" ]; then
        printf 'error: remote archives require --archive-sha256 unless --skip-checksum is used\n' >&2
        exit 1
      fi
  ...
  if [ -n "$ARCHIVE_SHA256" ]; then
    sha256_check "$ARCHIVE_SHA256" "$ASSET_PATH"
  fi
else
